...
export const MovieDetail = () => {
  ...
  const { movie } = useMovieDetail(param);
    ...
    <main>
      <section className="flex flex-wrap justify-around py-5">
        <div className="max-w-sm">
          <img src={imagePath} alt={movie.original_title} className="rounded" />
        </div>
        <div className="max-w-2xl text-gray-700 dark:text-white text-lg">
          <h1 className="text-center text-4xl font-bold my-3 lg:text-left">
            {movie.original_title}
          </h1>
          <p className="my-4 text-center lg:text-left">{movie.overview}</p>
          {movie.genres && (
            <p className="text-center lg:text-left my-7">
